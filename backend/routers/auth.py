...
from ..db.auth_queries import create_user, get_user_by_username
...
async def register(body: AuthRequest, conn=Depends(get_db)):
    ...
    if existing:
        return JSONResponse(status_code=409, content={"ok": False, "error": "用户名已被注册"})

    hashed = _hash_password(body.password)
    user = create_user(conn, body.username, hashed)
    if user is None:
        return JSONResponse(status_code=500, content={"ok": False, "error": "注册失败，请稍后重试"})
...
async def login(body: AuthRequest, conn=Depends(get_db)):
    ...
    if row is None:
        return JSONResponse(status_code=401, content={"ok": False, "error": "用户名或密码错误"})
